...
let _ = let rstart = ref (setup ()) in
  ...
  let rec handle_event rs =
    let evstat = Graphics.wait_next_event [Graphics.Button_down; Graphics.Key_pressed; Graphics.Mouse_motion] in
    (if evstat.keypressed then (Mutex.lock mt; rs := key_pressed !rs evstat.key; Mutex.unlock mt)
     else if evstat.button then (Mutex.lock mt; rs := mouse_clicked !rs; Mutex.unlock mt)
     else (Mutex.lock mt; rs := mouse_moved !rs; Mutex.unlock mt);
     handle_event rs) in
